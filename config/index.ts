...
const TextInput = defineAsyncComponent(
  () => import('../components/Inputs/TextInput/TextInput.vue'),
)

const NumberInput = defineAsyncComponent(
  () => import('../components/Inputs/NumberInput/NumberInput.vue'),
)
...
export const appConfig = {
  ...
  table: {
    ...
    extendTableFetchInput: (fetchInput: ITableDataFetchFncInput) => {
      const { fetchTableQuery } = fetchInput

      fetchInput.prismaQuery = {
        includeCount: fetchTableQuery.isInitialFetch,
        includeModel: fetchTableQuery.isInitialFetch,
        search: fetchTableQuery.search,
        args: {
          where: transformToPrismaWhere(fetchTableQuery.filters),
          orderBy: transformSortingToPrismaOrderBy(fetchTableQuery.orderBy),
          select: transformToPrismaSelect(fetchTableQuery.select),
          take: fetchTableQuery.take,
          skip: fetchTableQuery.skip,
        },
      }
    ...
    layoutsKey: 'data',
    defaultLayoutKey:
      'data.additionalInformation.filtersMeta.data.defaultFilter',
    hashKeys: {
  ...
  dataTypes: {
    comparators: {
      ...
      enumSimple: [ComparatorEnum.EQUAL, ComparatorEnum.NOT_EQUAL],
      numberArray: [
        ComparatorEnum.EQUAL,
        ComparatorEnum.NOT_EQUAL,
      ...
      stringArraySimple: [ComparatorEnum.EQUAL, ComparatorEnum.NOT_EQUAL],
    },
    inputs: {
      enum: {
        component: TextInput,
      ...
      numberArray: {
        ...
        icon: 'i-material-symbols:data-array-rounded',
        props: {},
      },
      numberArraySimple: {
        component: NumberInput,
    ...
    selectorComparators: [ComparatorEnum.IN_EVERY, ComparatorEnum.IN_NONE],
    nonValueComparators: [],
  ...
  error: {
    handler: (error: any, t: any): string[] => {
      if (error) {
        const trpcErrors = error.message.split('\n')

        // // TRPC errors
        if (trpcErrors.length) {
          return trpcErrors
        }
  ...
  request: {
     ...
     */
    modifyFnc: resolveLocalizationStrings,
