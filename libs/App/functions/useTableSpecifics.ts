...
export function useTableSpecifics() {
  ...
  const { tableLayoutFindMany, tableLayoutUpsertOne, tableLayoutDeleteOne }
    = useTableLayoutApi()
  ...
  function saveLayout(
    ...
    return tableLayoutUpsertOne({
      create: {
        ...payload,
        user: { connect: { id: currentUser.value?.id } },
      },
      update: payload,
      where: { id: Number(layout.id ?? -1) },
      select: { id: true, name: true, schema: true, tableName: true },
    })
  }
  ...
  async function deleteLayout(id?: number | string, tableName?: string) {
    ...
    const res = await tableLayoutDeleteOne({
      where: { id: +id },
      select: { id: true },
    })
